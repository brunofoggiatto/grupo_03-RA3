...
std::unique_ptr<CGroupMetrics> CGroupManager::getSystemMetrics() const {
    ...
    if (metrics->cgroup_version == "v2") {
        ...
        if (fileExists("/sys/fs/cgroup/memory.failcnt")) {
            std::string mem_failcnt = readFile("/sys/fs/cgroup/memory.failcnt");
...
// Função ADICIONAL para ler métricas de um cgroup específico
